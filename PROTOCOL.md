...
**一区** — 2×3×2 编码（取杆×抬升×换杆，三排独立）：

```
matrixVal = takePos × 6 + liftPos × 2 + rodPos
```

| 参数 | 范围 | 值 |
|------|------|-----|
| takePos | 0~1 | 0=取杆, 1=收杆 |
| liftPos | 0~2 | 0=低, 1=中, 2=高 |
| rodPos | 0~1 | 0=换杆空(默认), 1=换杆 |

换杆空默认始终选中；按下换杆时 rodPos=1，松开恢复为 0。
...
**三区** — 4×3 编码（上排 4 选 1 + 下排 3 选 1）：

```
matrixVal = topPos × 4 + bottomPos
```

| 位 | 参数 | 值 |
|----|------|-----|
| bit3:2 | topPos | 0=R2高, 1=R2低, 2=攻击低, 3=攻击高 |
| bit1:0 | bottomPos | 0=放块, 1=捡块, 2=放杆 |

组合示例：

| topPos | bottomPos | matrixVal | 含义 |
|--------|-----------|-----------|------|
| 0 | 0 | 0 | R2高 + 放块 |
| 1 | 2 | 6 | R2低 + 放杆 |
| 3 | 1 | 13 | 攻击高 + 捡块 |
...
| 区域 | 规则 |
|------|------|
| 一区 | cell[0~1] 二选一，cell[2~4] 三选一，cell[6~7] 二选一（换杆空默认选中） |
| 二区 | cell[0~5] 六选一 |
